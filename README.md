...
[![GitHub all releases](https://img.shields.io/github/downloads/BoringMan314/bm-mouse-click/total)](https://github.com/BoringMan314/bm-mouse-click/releases)
...
- **更新檢查**（啟動時一次）：向 GitHub Releases 查詢是否有較新版本；有新版時**視窗標題**每 3 秒輪播提示，系統匣出現「下載更新」。**僅**點「下載更新」時，將 Win10 版 exe 存到程式同目錄，檔名為 `bm-mouse-click-Vx.y.z.exe`（不覆寫正在執行的 exe；同檔名已存在則跳過）。
